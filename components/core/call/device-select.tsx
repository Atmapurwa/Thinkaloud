...
const selectVariants = cva(
  [
    ...
    'w-full rounded-md px-3 py-2 text-sm cursor-pointer',
  ...
  return (
    <Select value={activeDeviceId} onValueChange={setActiveMediaDevice}>
      <SelectTrigger className={cn(selectVariants({ size }), props.className)}>
        {size !== 'sm' && (
          <SelectValue className="font-encode-sans text-sm" placeholder={`Select a ${kind}`} />
        )}
      </SelectTrigger>
      <SelectContent>
        {devices.map((device) => (
          <SelectItem key={device.deviceId} value={device.deviceId} className="font-encode-sans text-xs">
            {device.label}
          </SelectItem>
        ))}
      </SelectContent>
